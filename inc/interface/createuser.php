<?php
if (isset($_POST['token']) && $_POST['token'] == "crecpt"){
    $login = $_POST['cptname'];
    $mdp = $_POST['cptpass'];
    $type = $_POST['cpttype'];
    $passha = hash('sha256', $mdp);

    $ins = $dbh->prepare('INSERT INTO login (login, mdp, islocked, type)
    VALUES ( :login, :mdp, :lock, :type)');
    $ins->execute(array(
        'login' => $login,
        'mdp' => $passha,
        'lock' => 0,
    ...
    echo'<div id="cre" class="intframe" style="display:inline-block">
